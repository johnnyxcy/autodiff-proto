...
class Assignments:
    ...
    def __getitem__(self, node: Union[str, cst.CSTNode]) -> Collection[BaseAssignment]:
        ...
        return (
            set(self._assignments[name])
            if name and name in self._assignments
            else set()
        )
...
class Accesses:
    ...
    def __getitem__(self, node: Union[str, cst.CSTNode]) -> Collection[Access]:
        ...
        name = get_full_name_for_node(node)
        return self._accesses[name] if name and name in self._accesses else set()
...
class BuiltinScope(Scope):
    ...
    def __init__(self, globals: Scope) -> None:
        self.globals: Scope = globals  # pyright: ignore[reportIncompatibleVariableOverride] must be defined before Scope.__init__ is called
...
class IfElseScope(LocalScope):
    """
    When an if-else statement is defined, it creates an IfElseScope.
    """
...
class ScopeVisitor(cst.CSTVisitor):
    ...
    def visit_If(self, node: cst.If) -> Optional[bool]:
        self.provider.set_metadata(node.test, self.scope)
        with ExitStack():
            with self._new_scope(IfElseScope, node.body, get_full_name_for_node(node)):
                node.body.visit(self)
                if_assignment_names = set([ass.name for ass in self.scope.assignments])

            if node.orelse:
                with self._new_scope(
                    IfElseScope, node.orelse, get_full_name_for_node(node)
                ):
                    node.orelse.visit(self)
                    orelse_assignment_names = set(
                        [ass.name for ass in self.scope.assignments]
                    )
                for name in if_assignment_names.intersection(orelse_assignment_names):
                    self.scope.record_assignment(name, node)
